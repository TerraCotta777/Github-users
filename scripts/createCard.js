export const createCardUser = (
  ...
  if (id in faves) {
    isFave.classList.add("card__is-fav", "true");
  } else {
  ...
  isFave.addEventListener("click", addToFaves);
...
export const createCardRepo = (name, url) => {
  const card = document.createElement("div");
  card.classList.add("repositories__card", "card");
  card.innerHTML = ` <p>${name}</p><a href='${url}' class="card__button button">Go to Github</a>`;
  return card;
};
